<?php
	...
	if(!class_exists('Agentes')) require 'Agentes.php';
...
switch($_SERVER['REQUEST_METHOD']){
    case 'DELETE':{
		...
		if(isset($_RECV['key']) && $_RECV['key'] == 'PJI310'){
			...
			if(!empty($u)){
				$rs = Visitas::apagarVisita($u, $_RECV);
	...
	case 'PUT':{
		...
		if(isset($_RECV['key']) && $_RECV['key'] == 'PJI310'){
			...
			if(!empty($u)){
				$rs = Visitas::alterarDados($u, $_RECV);
	...
	case 'GET':{
		...
		if(isset($_RECV['key']) && $_RECV['key'] == 'PJI310'){
			...
			$rs = $id > 0 ? Visitas::getVisita($id) : Visitas::listarVisitas();
	...
	case 'POST':{
		...
		if(isset($_RECV['key']) && $_RECV['key'] == 'PJI310'){
			...
			if(!empty($u)){
				$rs = Visitas::cadastrarVisita($u, $_RECV);
