   ...
   case 'PUT':{
	...
	case 'GET':{
		...
		if(isset($_RECV['key']) && $_RECV['key'] == 'PJI310'){
			...
			$rs = $id > 0 ? Pacientes::getPaciente($id) : Pacientes::listarPacientes();
	...
	case 'POST':{
		...
		if(isset($_RECV['key']) && $_RECV['key'] == 'PJI310'){
			...
			if($id > 0){
				$rs = Pacientes::loginPaciente($_RECV);
				$u = $rs!=null && is_numeric($rs) ? new Pacientes($rs) : array();
				...
				$rs = Pacientes::cadastrarPaciente($_RECV);
